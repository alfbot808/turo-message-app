...
export const faqTemplates = {
  // PICKUP / LOCATION
  lockboxLocation: `The lockbox is attached to the driver's side window. You'll find the key inside once you enter the code I provided.`,

  airportParkingFees: `No need to pay for parking! The fee is already covered through Turo. Please exit through the CASH lane and show your reservation to the gate attendant if needed.`,

  terminal1Warning: `🚨 IMPORTANT: When entering the International Parking Garage, use the LEFT ticket arm (labeled "International"). DO NOT use the right entrance (Terminal 1) — fees will be charged and you'll be responsible for reimbursement.`,

  // VEHICLE OPERATION
  teslaHowToStart: `To start the Tesla:
1. Tap the key card on the center console (behind the cup holders)
2. Press the brake pedal
3. Select Drive or Reverse on the touchscreen

The car is always "on" when you're in it. Just press the brake and go!`,

  teslaCharging: `⚡️ Tesla Charging:
...
Keep charge between 20-80% during your trip. Check your hotel — many offer free charging!`,

  sonataFuelType: `The Hyundai Sonata Hybrid takes regular unleaded gas (87 octane). It's a hybrid, so you'll get excellent mileage — expect 50+ MPG!`,

  sonataHowToStart: `The Sonata has a push-button start:
1. Make sure the key fob is inside the car
2. Press the brake pedal
3. Press the Start/Stop button on the dashboard`,

  // TRIP MODIFICATIONS
  extendTrip: `To extend your trip, please use the "Change trip" feature in the Turo app. Go to Trips → Select your trip → Change trip. I'll need to approve the request, so please give me a heads up once you submit it!`,

  addDriver: `Yes! You can add another driver at no charge. They must:
1. Create a Turo account
2. Get approved to drive
3. Be added to your trip at least 24 hours before pickup

Go to Trips → Select your trip → Add driver to submit their info.`,

  // DURING TRIP ISSUES
  flatTire: `Oh no! Here's what to do:
...
Stay safe first, worry about the car second.`,

  lockedOut: `I can help you get back in! For the Tesla, I can unlock it remotely through the app — just message me and I'll unlock it immediately.

For the Hyundai, you'll need to use the spare key in the lockbox (if you still have access) or call a locksmith. Turo roadside assistance can also help: 1-888-391-0460`,

  checkEngineLight: `If a warning light comes on:
...
Don't ignore red warning lights — better to check than cause damage.`,

  // RETURN / CHECKOUT
  returnFuelLevel: `Tesla: Return with battery between 20-80% charge.

Sonata: Return with the same fuel level as pickup. If you forget, I'll request reimbursement for the missing fuel plus a $10 inconvenience fee per Turo policy.`,

  returnLocationReminder: `Airport returns: International Parking Garage, Level 7 (near elevators). Remember — use the LEFT entrance (International), NOT Terminal 1!

Video guide: https://youtu.be/Sm2zpl12iXA?si=Ik61NXL3YEbZDZ1B

Kaneohe returns: 45-315 Lilipuna Rd, Stall 19`,

  lateReturn: `Please message me ASAP if you're running late. Turo charges late fees:
• Up to 1 hour late: No charge (if you communicate)
• 1-2 hours: $50/hour
• 2+ hours: Full extra day charge

I can often work with you if you give me a heads up before the scheduled return time!`,

  // POST-TRIP
  leftItemInCar: `No problem! Let me know:
...
I check the car thoroughly after each trip, so I likely have it.`,

  ticketToll: `If you got a ticket or toll:
...
Unpaid tickets can result in additional fees, so handling them quickly helps us both.`,

  // POLICY
  smoking: `🚭 Strict no-smoking policy in all our vehicles. This includes:
• Cigarettes
• Vapes/e-cigarettes
• Marijuana (even though it's legal in HI)

Violations result in a $150+ cleaning fee per Turo policy. Please smoke outside the vehicle and away from it to avoid odor transfer.`,

  cleaning: `Normal cleaning is included (dust, light vacuuming). But you may be charged if:
• Excessive sand/dirt
• Food stains or spills
• Pet hair (if no pet add-on)
• Smoke odor

Quick tip: Brush off sand at the beach before getting in, and use a towel if you're wet. Saves us both hassle!`,

  petPolicy: `Pets are allowed with the pet add-on! Without it:
• $150+ cleaning fee for pet hair/dander
• Damage charges for scratches, odors, etc.

With the add-on: Just return the car reasonably clean. I love pets, just need to protect the next guest with allergies.`,

  mileage: `Your trip includes the mileage limit shown in your booking. If you go over:
• $0.50 per mile for the Tesla
• $0.40 per mile for the Sonata

These charges are processed automatically through Turo after your trip ends.`,
};

export const messageTemplates: MessageTemplates = {
  tesla: {
    airport: {
      booking: `Aloha [Guest Name]!!
...
Taylor & Madi
Turo Hosts`,
      pickup: `Aloha and welcome to Honolulu! 🌺
